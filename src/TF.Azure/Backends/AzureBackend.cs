...
public class AzureBackend : Backend
{
        ...
        string containerName, string blobName) : base(credential)
    {
        ...
        var storageAccount = new ResourceIdentifier(storageAccountResourceId);
        if (storageAccount.ResourceType.ToString() != storageAccountResourceType)
            throw new Exception($"Provided resourceId isn't of type '{storageAccountResourceType}'");

        if (string.IsNullOrWhiteSpace(storageAccount.SubscriptionId))
            throw new Exception("Provided storage account resourceId does not contain a subscription id.");

        if (string.IsNullOrWhiteSpace(storageAccount.ResourceGroupName))
            throw new Exception("Provided storage account resourceId does not contain a resource group name.");
